...
This document describes the main LangGraph graph implemented in
`graph_code.agent.graph.build_agent()`. It is the authoritative map for the
agent loop, routing decisions, interrupt/resume behavior, and tool-result
message flow.
...
- `graph_code/main.py`: drives `graph.stream(...)` and
  `Command(resume=...)` from the CLI.
...
`AgentState.messages` uses LangGraph's `add_messages` reducer. This is
important because assistant tool calls and `ToolMessage` results must stay in
the message history without manual duplication.
...
Compatibility fields such as `tool_calls`, `iteration_count`,
`pending_question`, `pending_confirmation`, `final_response`, and `error` are
kept for the existing public API and tests.
...
The development default is memory-backed persistence. Non-memory backends are
created through `graph_code.agent.persistence`, leaving SQLite/Postgres paths
...
| --- | --- | --- |
| `drain_notifications` | `drain_notifications` | Marks whether queued notifications exist before prompt/model work. |
...
| `recovery_handler` | `recovery_handler` | Handles model errors and transient retry routing before the model response is routed. |
| `route_model_response` | `lambda state: {}` | Empty node used as a named routing point after recovery. |
| `permission_gate` | `permission_gate_node` -> `permission_gate` | Evaluates tool permissions and creates permission requests, denied envelopes, or executable calls. |
...
This route is what prevents one approval from authorizing every side-effecting
tool in the same model response.
...
The second `call_model` sees the original assistant tool call plus matching
`ToolMessage` entries and can produce a final response or request more tools.
...
Approved tool calls are stored in `approved_tool_calls`. If more pending tool
calls remain, the graph routes back to `permission_gate` so each risky tool is
checked independently. When no more approvals are needed, approved calls move
to `execute_tools`.

### 4. Tool Denial

If the user denies a permission request, `human_permission_interrupt` creates a
normal error `ToolResultEnvelope` for that specific `tool_call_id`.

Denied results do not crash the graph. They are appended as `ToolMessage`
objects so the model receives a valid response for the denied tool call.
...
`recovery_handler` checks `recovery_state["transient_retry_budget"]`. If budget
remains, it clears `error`, decrements the budget, writes
`transition_reason = "transient_model_retry"`, and `route_model_response`
routes back to `call_model`.

### 6. Context Compaction Loop

After tool results are appended, `compact_check` may summarize long history.
The graph then passes through `recovery_handler_after_tools` and returns to
`call_model`. This keeps compaction inside the normal tool-result loop.
...
`call_model` validates this before hitting the provider. If the local history
is invalid, the graph returns `transition_reason = "message_protocol_error"`
instead of sending a request that would fail with a provider-side 400.

`append_tool_results` is the only graph node that writes tool results back into
`messages`. It converts each `ToolResultEnvelope` into a `ToolMessage`.

## Tool Result Ordering

`execute_tools` merges newly executed results with pre-existing denied results
and sorts them according to the latest assistant tool-call order. This keeps
tool results stable even when some calls are denied and others are executed.
...
The runner synchronizes streamed state updates back into the caller-held
`state` with `_sync_state_update`. This is required because the CLI keeps a
local state object across turns while LangGraph also stores checkpointed state.
...
This avoids the apparent hang where a tool has finished but the graph is
waiting on the next model call.
...
This keeps `run_agent` and `run_agent_async` behavior aligned and prevents stale
per-turn state from leaking into the next request.

## Design Invariants

- The graph runtime is LangGraph's `StateGraph`; this project does not
  reimplement checkpointing, interrupt, or streaming runtime behavior.
- Every user turn and resume path supplies `thread_id`.
- Human approval is implemented with LangGraph `interrupt()` and
  `Command(resume=...)`.
...
These tests cover stream state sync, permission interrupts/resume, multiple
side-effect approvals, tool result ordering, message protocol validation,
transient retry, and CLI progress rendering.
